...
const werewolfTurn = async (players, deads, werewolfChannel) => {
    ...
    const collector = werewolfChannel.createMessageComponentCollector({
        filter: i => i.isStringSelectMenu() && i.customId === 'Werewolves'
            && !voted.find(p => p === i.user.username)
    ...
    collector.on('collect', async i => {
        ...
        voteDisplay.push(`${i.user.username} à voté pour ${vote}`);
        ...
        if (voted.length === nbWerewolves) {
            ...
            werewolfChannel.send(`Vous avez tué ${randomlySelected.vote} avec ${maxVotes} votes`);

            deads.push(players.find(u => u.player.username === randomlySelected.vote));
        }
    });
}
